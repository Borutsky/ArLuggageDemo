                ...
                sceneView.arFrame?.let { frame ->
                    if (frame.camera.trackingState == TrackingState.TRACKING) {
                        ...
                        if (hitTestIterator.hasNext()) {
                            if (!placed) {
                                // Getting the biggest plane in the session by its area
                                val bestPlane =
                                    sceneView.session?.getAllTrackables(Plane::class.java)
                                        ?.maxBy { it.extentZ * it.extentX }
                                // Getting Y of the biggest plane
                                val bestPlaneY = bestPlane?.centerPose?.ty() ?: 0f
                                // Searching for the closest hit result to the biggest plane by Y
                                val hitResult =
                                    hitTestIterator.asSequence()
                                        .sortedBy { abs(it.hitPose.ty() - bestPlaneY) }.first()
                                val hitPose = hitResult?.hitPose
                                val anchor = sceneView.session?.createAnchor(
                                    Pose(
                                        floatArrayOf(
                                            hitPose?.tx() ?: 0f, // X of the best hit result
                                            bestPlaneY ?: 0f, // Y of the biggest plane
                                            hitPose?.tz() ?: 0f // Y of the best hit result
                                        ), floatArrayOf(0f, 0f, 0f, 0f) // No rotation
                                    )
                                )
                                if (anchorNode == null) {
